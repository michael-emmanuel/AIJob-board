...
export const helloWorld = inngest.createFunction(
  ...
  async ({ event, step }) => {
    await step.sleep('wait-a-moment', '1s'); // add 's' for seconds
...
export const handleJobExpiration = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.sleep('wait-for-expiration', `${expirationDays}d`); // add 'd' for days
...
export const sendPeriodicJobListings = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    while (currentDay < totalDays) {
      await step.sleep('wait-interval', `${intervalDays}d`); // 15 emails total
      ...
      if (recentJobs.length > 0) {
        await step.run('send-email', async () => {
          ...
          await resend.emails.send({
            from: 'CareerSyncAI <onboarding@careersync.ai>',
            to: [email],
            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="margin-top: 30px; text-align: center;">
                ...
                style="background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">View More Jobs</a>
              </div>
            </div>  
            `,
          });
